...
const TotalYearRevenuesModal = ({modal, setModal, item, setShouldUpdate})=>{
    ...
    return(
        <Modal className={lang === 'en' ? "en fade addRevenues" : "ar fade addRevenues"} style={{textAlign: lang === 'en' ? 'left' : 'right'}} show={modal} onHide={()=>{
            ...
            <Modal.Header>
            <Modal.Title>{Translate[lang]?.total} {Translate[lang]?.year_revenues}</Modal.Title>
            <Button
                variant=""
                className="close"
...
export default TotalYearRevenuesModal;